...
export class Gantt3Component {
  ...
  checkSession() {
    if(this.sessionStorage.getDataFromSessionStorage('projetWorking') == null) {
      ...
      this.sprintService.getSprintByIdProjet(this.idProjet).subscribe( sprints => {
        for(let i = 0; i < sprints.length; i++) {
          if(sprints[i].date_debut) {
            sprints[i].date_debut = new Date(sprints[i].date_debut).toLocaleDateString();
            const [day, month, year] = sprints[i].date_debut.split('/');
            sprints[i].date_debut = `${month}/${day}/${year}`;
          }
          if(sprints[i].date_fin) {
            sprints[i].date_fin = new Date(sprints[i].date_fin).toLocaleDateString();
            const [day, month, year] = sprints[i].date_fin.split('/');
            sprints[i].date_fin = `${month}/${day}/${year}`;
          }
        }
        this.sprintsProjet = sprints;
        console.log(this.sprintsProjet)
      });
      this.backlogService.getBacklogByIdProjet(this.idProjet).subscribe( backlogs => {
        this.backlogsProjet = backlogs;
        let backlogId:number[] = [];
        for(let backlog of backlogs) backlogId.push(backlog.id)
        this.fonctionnaliteService.list().subscribe( fonctionnalites => {
          ...
          this.tacheService.list().subscribe( taches => {
            ...
            this.sprintsProjet.forEach(sprints => {
              if(sprints.date_debut && sprints.date_fin && sprints.date_debut <= sprints.date_fin) {
                console.log(sprints)
                const { idSprint, nom, date_debut, date_fin } = sprints;
                const dure = Math.ceil(Math.abs((new Date(date_debut).getTime() - new Date(date_fin).getTime()) / (1000 * 3600 * 24)));
                console.log(dure);
                const taskData: {
                  id: number;
                  nom: string;
                ...
                console.log(new Date(new Date(date_debut).setDate(new Date(date_debut).getDate() + dure)))
              
                this.backlogsProjet.forEach(backlogs => {
                  if (backlogs.dateDebut && backlogs.dateFin && backlogs.dateDebut <= backlogs.dateFin && backlogs.sprint?.idSprint === idSprint) {
                    console.log(backlogs)
                    const { id, nom, dateDebut, dateFin } = backlogs;
                    const backlogData = {
                      id: id, nom: nom, date_debut: new Date(new Date(dateDebut).toLocaleString('en-US', {month: '2-digit',day: '2-digit',year: 'numeric'})), date_fin: new Date(new Date(dateFin).toLocaleString('en-US', {month: '2-digit',day: '2-digit',year: 'numeric'}))
                    };
