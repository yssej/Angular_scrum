  ...
  setStoryPoint(content:any) {
    if(this.projetInWork.dateDebut) {
      ...
      if(backlogs.length < 1) {
        ...
        let enddd:number = new Date(this.projetInWork.dateDebut).getDate() + this.backlogSelected.storyPoint - 1;
        ...
        if(!backlogs.filter(resp => resp.dateDebut != this.projetInWork.dateDebut)) {
          ...
          enddd = new Date(backlogs[backlogs.length - 1].dateFin).getDate() + this.backlogSelected.storyPoint;
